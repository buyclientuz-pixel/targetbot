...
| `project:leads:{status}:{projectId}:{periodKey}[:from][:to]` | Открыть панель лидов проекта с выбранным статусом и периодом (`periodKey = today/week/month/all/custom`). |
| `project:leads-target:{status}:{projectId}:{channel}:{periodKey}[:from][:to]` | Переключить канал рассылки лидов для статуса (`status = new/processing/done/trash`, `channel = chat/admin`) и перерисовать текущий период. |
| `project:leads-range:{status}:{projectId}` | Запросить ручной ввод периода для панели лидов. |
...
- 2025-03-05 — Панель лидов поддерживает периоды: `project:leads:{status}:{projectId}:{periodKey}[:from][:to]`, отдельные переключатели каналов (`project:leads-target:*`) и новый `project:leads-range:{status}:{projectId}` для ручного ввода дат.
- 2025-02-26 — Добавлен `project:leads-target:{status}:{projectId}:{channel}` для управления уведомлениями о лидах в Telegram-панели (кнопки «👥 Чат / 👤 Админ»).
