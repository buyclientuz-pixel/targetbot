...
| `proj:billing-next:{projectId}:{preset}` | Обновить дату следующего платежа (`30`, `custom`, `manual`, `today`, `yesterday`). |
...
- 2025-02-24 — Раздел «💰 Оплата» переведён на специфицированные пресеты: используются `proj:billing-next:{id}:{30|custom|manual|today|yesterday}` и `proj:billing-tariff-preset:{id}:{amount}` для тарифов 350$/500$.
