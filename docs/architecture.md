...
4. **Отчёты** — `/api/reports` добавляет CSV в `R2_REPORTS`, `/report` в Telegram выводит список актуальных выгрузок.
5. **Админ-панель** — фронтенд забирает данные с `/api/leads`, `/api/users`, `/api/settings`, обновляет метрики в реальном времени.
