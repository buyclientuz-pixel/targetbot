...
1. Получите Bot Token у `@BotFather` и убедитесь, что секрет сохранён как переменная окружения `TELEGRAM_BOT_TOKEN` в Cloudflare Worker (например, `wrangler secret put TELEGRAM_BOT_TOKEN`).
...
Если `ok = false`, проверьте корректность токена и доступность URL (должен отвечать с кодом 200).
...
3. Дополнительно отправьте сообщение боту и убедитесь, что в логах Cloudflare Worker (или через `wrangler tail`) появляются события без ошибок `Invalid Telegram payload`.
