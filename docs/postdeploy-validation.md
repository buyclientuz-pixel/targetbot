...
2. При необходимости повторите OAuth-флоу (`/api/meta/oauth/start` → авторизация → `/auth/facebook/callback`).
...
| Планировщик автоотчётов выполнил слот по расписанию (проверить логи) |  |  |
