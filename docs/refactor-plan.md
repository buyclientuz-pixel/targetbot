...
- Настроить отправку уведомлений о лидах напрямую из webhook с учётом project-settings.chatId/topicId.
...
## Итерация 8 — Автоотчёты
- ✅ Реализованы cron-триггеры Workers, автоотчёты по таймслотам и маршруты отправки с независимыми каналами (CHAT/ADMIN).
- ✅ Добавлены состояния `report-state:*` для дедупликации автоотчётов и контроля слотов.
- ✅ Автоматическое создание темы «Таргет» и кеш статусов кампаний для карточек проекта.
...
- ✅ Описаны мониторинг и SLA для автоотчётов.
