...
- Выполнено: восстановлено отображение подключённых проектов — бот объединяет записи из R2, KV и зеркала Meta, разделы «📊 Проекты» и карточки работают как в Telegram, так и на веб-панели.
- Осталось: провести финальные регрессионные проверки отчётности и подготовить релизную документацию.
- Итераций до полного покрытия ТЗ: ≈1 — заключительный QA и выпуск отчётного пакета.

### Текущее состояние задач

**Выполнено в этой итерации**
- Список проектов теперь объединяет данные из R2, KV и зеркала `meta_projects`, поэтому действующие проекты снова видны в Telegram и REST-API.
- Коллбеки `cmd:projects`/`proj:view:{id}` возвращают полноценную карточку проекта с лидами, биллингом и переходами, включая уведомления привязанным чатам.
- Веб-панель /admin отображает подключённые проекты с кнопками редактирования и настроек, используя общие сводки `summarizeProjects`.

**Осталось закрыть**
- Закрыть smoke-/regression-тестирование отчётности и напоминаний на общей инфраструктуре.
- Подготовить релизный runbook с контрольным чек-листом по автоотчётам и SLA-экспортам.
