...
- **Следующее:** Итерация 3 master-spec rollout — перенос клиентского портала и API на master-spec (прелоадер, табы метрик, оплаты, экспорт) и прогон ручной приёмки портала/бота/вебхуков.
