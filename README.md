...
- Выполнено: STEP 14 перестроил модуль «Рекламные кампании» — `normalizeCampaign`/`compareCampaigns` дают единый порядок ACTIVE → PAUSED → другие статусы, подставляют objective-driven result KPI и короткие имена, а бот, портал и отчёты читают одну и ту же структуру.
- Осталось: провести финальный smoke/QA отчётности и оплаты, обновить runbook/deployment checklist перед релизом SPEC-AUTO-REPORT-v3.
- Итераций до полного покрытия ТЗ: ≈0 — финальная проверка и выпуск.

### Текущее состояние задач

**Выполнено в этой итерации**
- `normalizeCampaign`/`normalizeCampaigns` приводят Meta-кампании к единому виду: сортировка ACTIVE → PAUSED → остальное, shortName из первых 4–5 символов, result KPI из objective (лиды, сообщения, клики, покупки и т.д.).
- Telegram-бот в разделе «Рекламные кампании» выводит подробные строки (статус, result, расход, показы, клики), inline-кнопки работают с тем же порядком, а отчёты и автоотчёты используют обновлённые CampaignReportBlock.
- Клиентский портал сортирует кампании по статусу/расходу, показывает колонку «Результат», приводит таблицу лидов к формату Имя/Телефон/Тип/Дата/Кампания и использует короткие названия.
- Синхронизация лидов и webhook Meta сохраняют `campaignName`/`campaignShortName`, уведомления в темах групп включают тип заявки и короткое имя кампании, inline-кнопки «Позвонить»/«Открыть профиль» остаются без переходов.
- Во всех отчётах (summary, weekly, portal, ручные) и экспортных сценариях используется result KPI: текстовые блоки отображают «Целевой показатель» с числом, а выбор кампаний в портале/боте опирается на те же данные.
