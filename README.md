...
## Smoke-проверка Meta Graph API
Подробный отчёт — в [docs/meta-api-smoke.md](docs/meta-api-smoke.md).

- 2025-02-12 (UTC): запросы `me/adaccounts` и `<BUSINESS_ID>/owned_ad_accounts` возвращают статусы `account_status: 1/2`, которые соответствуют бейджам «Активен» и «Неактивен (UNSETTLED)» в админке.
- Ответ `GET /api/meta/adaccounts?include=raw` совпадает с исходными значениями Graph API, отличий по валюте и ID не обнаружено.
- Проверка `debug_token` подтвердила валидность long-lived токена и необходимые скоупы (`ads_management`, `business_management`, `pages_read_engagement`).
...
### Progress 25
- Что сделано: Проведены итоговые smoke-запросы к Meta Graph API; результаты сопоставлены с `/api/meta/adaccounts` и задокументированы в `docs/meta-api-smoke.md` и разделе README.
- Какая задача сейчас в работе: Финальные проверки — сбор логов build/deploy.
