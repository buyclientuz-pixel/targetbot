...
- Главное меню Telegram-бота переведено на inline-клавиатуру с кнопкой-ссылкой «Авторизация Facebook» (ведёт на HTML-роут `/fb-auth`), дублирующими `cmd:*`-callback'ами и корректной проверкой вебхука через `cmd:webhooks` без `https://https://` в ссылке; маршрут `/fb-auth` выдаёт пошаговую инструкцию по получению токена.
