...
- **Подключение через кнопки:** при запуске мастера показываются инлайн-кнопки со свободными рекламными аккаунтами Meta и
  зарегистрированными чатами (через `/register`), поэтому ID `act_…` и параметры чата не нужно копировать вручную.
