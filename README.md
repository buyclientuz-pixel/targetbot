# Targetbot — этап 5: карточка проекта

Проект «CI → GitHub → Cloudflare Workers → Telegram-бот отчётов по рекламе Meta» развивается поэтапно. После каркаса и базовой
логики Telegram реализован первый рабочий мастер создания проекта через админ-панель: код → выбор чата → выбор рекламного аккаунта.
...
- Список проектов получил кнопки «⚙️ код», открывающие карточку выбранного проекта. Карточка показывает аккаунт, чат,
  расписание, KPI, статусы сводника, автопаузы и алертов, а также даты оплат. Для будущих функций добавлены подсказки и
  кнопки-заглушки (расписание, KPI, биллинг, alerts, отчёт, кампании), чтобы видеть план расширения.
...
3. Расширить мастер проекта: добавить выбор расписания, периодов, KPI и статуса биллинга на этапе создания, а также редактор
   в карточке проекта (кнопки уже готовы и ждут реализации логики).
