...
- Выполнено: добавлен KPI-движок — бот сохраняет цели кампаний Meta (`campaign_objective:{project_id}:{campaign_id}`) и пользовательские наборы KPI (`project_campaign_kpis:{project_id}:{campaign_id}`), а также предоставляет мастер «🎛 KPI кампаний» для настройки прямо в Telegram; ссылки на чат-группы приводятся к формату `tg://openmessage?chat_id=…`, поэтому переход по числовым chat_id работает мгновенно.
- Осталось: финальный smoke/QA по расширенным KPI в отчётах и экспортах.
- Итераций до полного покрытия ТЗ: ≈0.2 — финальные проверки и документирование выгрузок.

### Текущее состояние задач

**Выполнено в этой итерации**
- Автосинхронизация целей кампаний Meta: при каждой загрузке списка кампаний заполняется KV-префикс `campaign_objective:*` и используется для предустановки KPI.
- Inline-мастер «🎛 KPI кампаний» с поддержкой многошагового выбора, поп-апами и сохранением как дефолтных, так и разовых наборов KPI (`report:kpi_*`).
- Портал, отчёты и автоэкспорты используют расширенный набор метрик (CTR/CPC/CPL/ROAS/CPM и др.) и отображают только выбранные KPI.
- Прогон `npm run typecheck` и `npm run qa` после обновления метрик — регрессионные сценарии зелёные.
- Кнопка «📲 Чат-группа» и все карточки проектов используют `tg://openmessage` для отрицательных `chat_id`, поэтому переходы в приватные супергруппы Telegram открываются без дополнительных ссылок.

**Осталось закрыть**
- Smoke-тесты CSV/HTML-экспортов с новым набором KPI и фиксация результатов в QA-журнале.
...
### Progress 49
- Что сделано: унифицированы ссылки на чат-группы — сохранённые `chat_id` превращаются в `tg://openmessage?chat_id=…`, бот мгновенно открывает приватные супергруппы и добавлены регрессионные тесты на нормализацию ссылок.
