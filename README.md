...
- [ ] Итерация 2 — Реализация Telegram-бота и главного меню
- [ ] Итерация 3 — Добавление Facebook OAuth и токен-менеджера
- [ ] Итерация 4 — Подключение Meta API, получение кампаний и расходов
- [ ] Итерация 5 — Создание логики проектов, лидов и оплат
...
| ------ | -------------- | -------- |
| Meta | `GET /api/meta/status`, `GET /api/meta/adaccounts`, `GET /api/meta/oauth/start`, `GET /api/meta/oauth/callback`, `POST /api/meta/refresh` | OAuth и выбор кабинетов |
...
- `/bot/webhook` приводит входящие апдейты к `BotContext` и разворачивает главное меню (итерация 1 — заглушки).
