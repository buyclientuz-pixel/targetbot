...
- Выполнено: STEP 15 обновил отчётный движок — автоотчёты/summary/ручные отчёты перешли на компактный текстовый шаблон за выбранный период, KPI выбираются по objective/override, кампаниям добавлены result-метрики и короткие имена, рассылки идут в нужную тему чата.
...
- Обновлён composeReportText/buildDatasetText: новые отчёты выводят ⏰/📅 заголовки, строки клиента, KPI (Лиды/CPA/Расход/CTR/CPC) и до пяти кампаний с result-метрикой и CPA.
- Автоотчётный движок маршрутизирует сообщения в чат-темы проектов (message_thread_id) и использует пресет `yesterday`, админам отправляется только при включённом канале.
- Регрессионные тесты подтверждают новый формат текста и inline-кнопок портала.
