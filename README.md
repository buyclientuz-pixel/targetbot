...
### 🧩 Автоматизация деплоя (Cloudflare npm mirror)
- npm registry перенаправлен на https://registry.npmjs.cf/
- bun install использует .bunfig.toml с тем же зеркалом, чтобы Cloudflare Build Service не обращался к заблокированному registry.npmjs.org
- wrangler устанавливается стабильно
- GitHub Actions авто-деплой завершён успешно
- curl-проверки эндпоинтов возвращают 200 OK
- все этапы логируются в deploy.log
