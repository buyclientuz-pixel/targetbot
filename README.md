...
- Реализован полный OAuth-flow (`/api/meta/oauth/start` → `/auth/facebook/callback`): короткий токен обменивается на long-lived, список рекламных аккаунтов сохраняется в `fb_auth:{userId}` и пользователю приходит уведомление в Telegram.
