...
- Выполнено: административные сценарии Telegram-бота очищены от упоминаний веб-панели, все подсказки направляют админов использовать меню бота; ограничения доступа для клиентских чатов обновлены соответствующим текстом.
- Осталось: контрольные smoke/QA после обновления копирайта и повторное подтверждение отчётов/экспортов.
- Итераций до полного покрытия ТЗ: ≈0.2 — завершающий раунд проверок и документирования.

### Текущее состояние задач

**Выполнено в этой итерации**
- Сообщения о недостающих проектах/лидах в отчётных мастерах теперь предлагают создать данные в боте, не упоминая внешние панели.
- Ограничения для клиентских чатов сообщают, что управление доступно только администраторам TargetBot, без ссылок на веб-интерфейс.
- Уведомление об успешной Meta OAuth подчёркивает, что все действия продолжатся в боте.
- Обновлена документация: README (Progress 50) и история callback-схемы, прогнаны `npm run typecheck` и `npm run qa`.

**Осталось закрыть**
- Повторное дымовое тестирование отчётов/экспортов после обновления копирайта и фиксация результатов в QA-журнале.

## Data Model (ER Diagram)

_Обновлено 2025-02-24: структура сущностей без изменений; пересмотрены только текстовые ответы бота._
