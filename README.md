...
- Telegram-бот реагирует на команды в группах объяснением, что управление доступно только в личном чате: если ЛС с ботом не открыт, пользователь получает подсказку прямо в группе (однократно с троттлингом), чтобы `/start`/`/admin` не выглядели «молчащими».
