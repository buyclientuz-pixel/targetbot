...
- Выполнено: восстановлена генерация публичных ссылок клиентского портала — бот строит URL из переменных окружения, авторизационного домена или workers.dev fallback, поэтому создание, перегенерация и рассылка портала остаются в той же карточке с pop-up подтверждениями.
- Осталось: финализировать релизный smoke-/release-чек-лист с учётом нового fallback и актуального журнала QA-прогонов.
- Итераций до полного покрытия ТЗ: ≈0.25 — завершаем релизную упаковку.

### Текущее состояние задач

**Выполнено в этой итерации**
- Клиентский портал получает рабочую ссылку сразу после создания — URL строится из `PORTAL_BASE_URL`/`PUBLIC_WEB_URL`/`WORKER_BASE_URL` либо из fallback на домен авторизации, поэтому интерфейс не заставляет выходить из карточки.
- Перегенерация ссылки, отправка портала и переключение режима (авто/ручной) отрабатывают внутри текущего сообщения с pop-up уведомлениями и мгновенной перерисовкой карточки.
- Ручные выборки кампаний сохраняются в настройках проекта, синхронизируются с порталом и включаются в автоотчёты без ручного дублирования.
- Регрессионный QA-раннер валидирует напоминания и расписания, журналирует результат и автоматически обновляет `nextRunAt` для просроченных задач.

**Осталось закрыть**
- Свести финальный smoke-/release-чек-лист по результатам `QA_RUNS` и подтвердить новые портальные ссылки в контрольных сценариях.
