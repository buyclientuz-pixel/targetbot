...
- Выполнено: кнопка «📲 Чат-группа» и все карточки проектов теперь используют нормализованные ссылки — бот автоматически добавляет `https://`/`tg://` к `t.me`, `@username`, `+invite` и числовым chat_id, поэтому переход в Telegram происходит без ошибок формата.
- Осталось: пройти контрольный прогон QA после обновления ссылок и подтвердить быстрый переход в боевых чатах.
- Итераций до полного покрытия ТЗ: ≈0.25 — завершаем релизную упаковку.

### Текущее состояние задач

**Выполнено в этой итерации**
- Поле `telegramLink` проектов очищается при сохранении: добавляются необходимые схемы (`https://`/`tg://`), поддерживаются `@username`, `t.me/...` и приглашения `+code`.
- Карточка проекта и раздел «📲 Чат-группа» используют нормализованный URL и fallback `https://t.me/c/<id>` для числовых идентификаторов, поэтому inline-кнопка сразу открывает нужный чат.
- Кнопки «📲 Чат-группа» в карточке, редакторе и списке действий остаются на текущем экране и выводят pop-up с результатом вместо отправки новых сообщений.
- Регрессионный QA-раннер остаётся источником истины: после изменений планируется контрольный прогон `npm run qa`.

**Осталось закрыть**
- Подтвердить обновлённые ссылки в smoke-/regression-сценариях и зафиксировать результат в QA-журнале.
