...
- Логин-панель админки теперь видна по умолчанию и не пропадает даже если скрипт не загрузился: после ввода `ADMIN_KEY` SPA разворачивает разделы, а при ошибках UI показывает статус «UI ошибка» и не скрывает форму.
...
- Веб-админка автоматически пробует API на текущем домене (`/api/admin`) и, при необходимости, повторяет запросы через `WORKER_URL`, добавляя заголовки `x-admin-key` и `Authorization: Bearer …`, поэтому авторизация по `ADMIN_KEY` больше не «застревает» даже при кастомных доменах.
...
1. Убедитесь, что в среде задан `ADMIN_KEY` и `WORKER_URL`. Если `ADMIN_KEY` не указан, воркер принимает дефолтный код `3590`.
...
3. Введите `ADMIN_KEY`; он сохраняется в `localStorage`, а все запросы панели отправляют заголовок `x-admin-key`.
   - Если ключ в `.env` записан в кавычках (`"!Lyas123"`), воркер автоматически их убирает: просто введите значение без кавычек.
   - При отсутствии переменной можно сразу использовать дефолтный код доступа `3590`.
