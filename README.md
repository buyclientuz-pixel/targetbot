...
- Выполнено: ручные фильтры кампаний и метрик теперь хранятся в настройках проекта, автоматически синхронизируют портал и подтягиваются в автоотчёты — расходы Meta считаются по выбранным кампаниям, а портал мгновенно обновляет режим и метрики без выхода из карточки.
- Осталось: прогнать полный регрессионный сценарий напоминаний и отчётных расписаний с учётом новых настроек и зафиксировать финальный QA-чек-лист перед релизом.
- Итераций до полного покрытия ТЗ: ≈1 — финализируем отчётность и контрольный QA.
...
- Переключатели метрик портала обновляют KV и сразу отражаются и в клиентском портале, и в отчётных пресетах.
- Автоотчёты запрашивают выбранные кампании в Meta, пересчитывают расходы и подтягивают их в Telegram-сводки.

**Осталось закрыть**
- Пройти сквозные проверки напоминаний и отчётных расписаний после обновлённого движка кампаний/метрик.
- Обновить финальный регрессионный чек-лист и зафиксировать результаты smoke-/regression-тестов перед релизом.
