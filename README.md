...
1. Убедитесь, что в среде задан `ADMIN_KEY` и `WORKER_URL`. Если `ADMIN_KEY` не указан, воркер принимает дефолтный пароль `ilyas123`.
...
3. Введите `ADMIN_KEY`; он сохраняется в `localStorage`, а все запросы панели отправляют заголовок `x-admin-key`.
   ...
   - При отсутствии переменной можно сразу использовать дефолтный пароль `ilyas123`.
