...
- Выполнено: карточка проекта получила отдельный экран редактирования с переименованием без выхода из бота и быстрыми ссылками на чат/отчёты, а раздел «⚙ Настройки» превратился в рабочие переключатели автоотчётов (ежедневно/еженедельно), тихих выходных, тихой отправки и lead-алертов — всё сохраняется в KV и мгновенно перерисовывает карточку.
- Осталось: синхронизировать кастомные метрики/кампании ручных отчётов с авторассылками и пройти итоговый регрессионный прогон напоминаний/экспортов.
- Итераций до полного покрытия ТЗ: ≈1 — завершаем отчётность и контрольный QA.

### Текущее состояние задач

**Выполнено в этой итерации**
- Раздел «✏️ Изменить данные» больше не заглушка: переименование запускается из карточки, ввод происходит в текущем чате с отменой и мгновенным обновлением проекта.
- Настройки проекта получили рабочие переключатели периода автоотчётов, тихих выходных, тихой отправки и lead-алертов; значения сохраняются в KV/R2 и отражаются в карточке.
- FSM дополнили хендлером отложенного ввода, поэтому бот корректно обрабатывает текст после callback без вылета в меню.

**Осталось закрыть**
- Связать ручной выбор метрик/кампаний с плановыми автоотчётами и порталом.
- Пройти сквозные проверки напоминаний и отчётов после обновления настроек.
- Обновить финальный регрессионный чек-лист перед релизом.
