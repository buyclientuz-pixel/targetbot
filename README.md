...
- Выполнено: добавлен автоматический QA-раннер (`npm run qa`), который проверяет напоминания и расписания отчётов, чинит просроченные `nextRunAt` и сохраняет результаты в журнал `QA_RUNS` (KV + R2).
- Осталось: подготовить финальный smoke-/release-чек-лист на основе свежих QA-прогонов и закрыть релизную документацию.
- Итераций до полного покрытия ТЗ: ≈0.5 — финальное оформление релиза.
...
**Осталось закрыть**
- Свести финальный smoke-/release-чек-лист по результатам `QA_RUNS` и зафиксировать его в документации перед релизом.
