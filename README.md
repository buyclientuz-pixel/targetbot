...
- **Клиентский портал:** защищённая ссылка `/p/{code}?sig=...` показывает агрегаты «Сегодня/Вчера/7 дней/МТД», дату следующей оплаты, топ-кампании и кнопку «Написать менеджеру» без раскрытия внутренних KPI.
...
2. Завершить мастер проектов в `/admin`: упрощённая привязка рекламных аккаунтов, чатов и расписаний из одного диалога.
3. Расширить отчёты: CSV-экспорт, пояснения для клиента, текстовые рекомендации и дополнительные каналы доставки.
4. Клиентский портал `/p/{code}` — базовая витрина готова, далее потребуется CSV-экспорт, пояснения для клиента и гибкая настройка токенов.
...
5. **Расширение клиентского портала** — добавить пояснения, экспорт в CSV и форму обратной связи для клиентов.
