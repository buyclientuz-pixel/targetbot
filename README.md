...
- Выполнено: мгновенное управление клиентским порталом прямо из карточки проекта (создание/перегенерация без выхода из меню, переключение режимов с попапами, шаринг в чат), обновлённый список проектов с расходами и прямыми ссылками в чат, быстрые тарифы/даты оплаты и автопроставление первой оплаты, а также автоматические попапы при отправке отчётов и скачивании экспортов.
- Осталось: выкатить расширенные настройки отчётности (ручной выбор метрик и кампаний внутри отчёта, обновление автоотчётов) и полный набор проектных автоматизаций.
- Итераций до полного покрытия ТЗ: ≈1 — финальная доводка настроек отчётности и QA.

### Текущее состояние задач

**Выполнено в этой итерации**
- Портал создаётся/перегенерируется без смены экрана, режимы Auto/Manual переключаются фоново с попапами, а шаринг портала сразу отправляет ссылку в чат клиента и обновляет карточку.
- Список проектов стал компактным: расход Meta выводится прямо в кнопках, а кнопка «📲 Чат-группа» ведёт напрямую в группу Telegram.
- Биллинг получил быстрые тарифы (350/500$), пресеты дат оплаты (сегодня/вчера/+30) и автозаполнение первой даты при создании проекта.
- Отправка отчётов в чат теперь возвращает к экрану отчётов с отметкой времени, а скачивание экспортов автоматически триггерит /admin через 5 секунд.

**Осталось закрыть**
- Добавить гибкие настройки метрик/кампаний для рекламных отчётов и синхронизацию автоотчётов с ручными настройками.
- Провести боевые проверки новых биллинговых пресетов и обновлённого портала на тестовых токенах и чатах.
- Зафиксировать регрессионный чек-лист для отчётных выгрузок (summary/auto/finance/SLA) перед релизом.
