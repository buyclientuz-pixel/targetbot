...
- Выполнено: cron-движок SPEC-AUTO-REPORT-v3 работает каждые 5 минут, Telegram-бот обслуживает `auto_*`/`alert_*` callbacks, summary подтягивает KPI/порталы и маршрутизируется по каналам; платёжные напоминания/подтверждения перенесены в бот с админскими и клиентскими маршрутами.
- Осталось: провести финальный smoke/QA отчётности, алертов и оплаты, обновить runbook/deployment чек-листы и зафиксировать результаты.
- Итераций до полного покрытия ТЗ: ≈1 — релизный QA SPEC-AUTO-REPORT и биллинга.

### Текущее состояние задач

**Выполнено в этой итерации**
- Cron-движок SPEC-AUTO-REPORT-v3 исполняет ежедневные/понедельничные отчёты и алерты (billing/budget/meta/pause) из Workers без участия веб-панели.
- Auto-report dataset объединяет KPI по целям кампаний, биллинг и траты проектов, добавляет портальные ссылки и inline-кнопки «Портал проекта».
- Авто-KPI движок подтягивает цели кампаний из Meta, позволяет настраивать дефолтные KPI на уровне проекта/кампаний и поддерживает разовые overrides в мастере отчётов.
- Telegram-бот отображает inline-панель автоотчётов с по-канальным управлением маршрутом (чат/админ), временем, понедельничным double-report и alert-триггерами `alert_*`.
- Автоотчёты и ручные отчёты используют новый текстовый шаблон без HTML-вложений, в рассылках остаются только inline-кнопки «Портал проекта», а кнопка «Скачать отчёт» и отправка HTML-файлов удалены.
- Платёжные напоминания автоматизированы: админские решения («Не продлеваю», «Продолжаем работу», подтверждение/ожидание/ошибка) и клиентские действия (наличные, перевод, подтверждение оплаты) выполняются внутри бота с обновлением KV.

**Осталось закрыть**
- Провести smoke/QA отчётных и платёжных сценариев, обновить runbook/deployment чек-листы и Progress Log по итогам тестов.
