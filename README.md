...
- Осталось: финализировать STEP 17 (итерация 3) — прогнать smoke/QA по сценарию оплаты, задокументировать cron-окна и edge-cases, подготовить релизную запись.
- Итераций до полного покрытия ТЗ: ≈1 (финальный QA/релиз).
...
- Планировщик `runReminderSweep` учитывает флаг `alerts.payment`, уведомляет админа за день до оплаты, повторяет follow-up при просрочке и автоматически блокирует отчёты/портал при отказе.
...
**Осталось закрыть**
- Провести финальный smoke/QA по биллингу и автоотчётам, зафиксировать результаты и обновить документацию перед релизом STEP 17.
