...
- Выполнено: ссылки на чат-группы нормализованы к формату `https://t.me/c/<chat>/<msg>` — кнопка «📲 Чат-группа» моментально открывает проектные супергруппы из карточки; обновлённые regression-тесты фиксируют формат.
- Осталось: контрольные smoke/QA после обновления формата ссылок и повторное подтверждение отчётов/экспортов.
...
- Кнопка «📲 Чат-группа» и связанные shortcut’ы используют `https://t.me/c/.../2`, что гарантирует переход в супергруппы без всплывающих диалогов Telegram.
- Нормализованы все сохраняемые `chat_id`/ссылки (включая старые `tg://openmessage`) при записи в KV/R2, чтобы существующие проекты тоже получили рабочий формат.
- Регрессионные тесты обновлены под новый формат ссылок.
- Обновлена документация: README (Progress 51) и история callback-схемы, прогнаны `npm run typecheck` и `npm run qa`.

**Осталось закрыть**
- Повторное дымовое тестирование отчётов/экспортов и быстрых переходов в чат после смены формата ссылок, фиксация результата в QA-журнале.
