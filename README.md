...
- Выполнено: внедрено управление клиентским порталом (создание, перегенерация, ручной выбор метрик и кампаний, мгновенный шэр в чат клиента), расширены проектные отчёты с выбором периода и отправкой в клиентский чат, добавлены мультивыбор и массовые действия по рекламным кампаниям.
- Осталось: прогнать e2e-проверки портала и массовых действий кампаний на стейджинге, убедиться в корректной доставке файлов отчётов для всех каналов.
- Итераций до полного покрытия ТЗ: ≈1 — финальный QA и релизная фиксация.

### Текущее состояние задач

**Выполнено в этой итерации**
- Реализован полный цикл клиентского портала в карточке проекта: генерация/перегенерация ссылок, ручной выбор метрик и кампаний, отправка портала и актуального отчёта в чат клиента.
- Проектные отчёты получили пресеты периодов (сегодня/вчера/неделя/месяц/всё время) и кнопку мгновенной отправки выбранного периода в клиентский чат.
- В разделе кампаний добавлены мультивыбор, массовое включение/отключение, сохранение набора для дефолтных отчётов и запуск моментального отчёта по выбранным кампаниям.

**Осталось закрыть**
- Провести боевые проверки порталов и массовых действий кампаний на тестовых токенах Meta и чатах Telegram.
