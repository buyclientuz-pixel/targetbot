...
- Выполнено: реализовано каскадное удаление проектов — бот отвязывает Meta-аккаунт, Telegram-группу, очищает лиды, платежи, отчёты и напоминания, а также уведомляет командный чат.
- Осталось: расширить клиентские отчёты/экспорты и провести финальные регрессионные проверки перед поставкой.
- Итераций до полного покрытия ТЗ: ≈1 — финальный акцент на отчётности и выпускных QA-процедурах.

### Текущее состояние задач

**Выполнено в этой итерации**
- Добавлен `deleteProjectCascade` в слой хранения: при удалении проекта очищаются KV/R2-записи, отчётные активы, расписания и напоминания.
- Telegram-бот получил мастер подтверждения удаления, каскадно отвязывает Meta-аккаунт/чат и рассылает уведомления администраторам и клиентскому чату.
- Документация и схема callback'ов обновлены: зафиксированы новые действия `proj:delete-confirm`/`proj:delete-cancel` и актуализирован прогресс итерации.

**Осталось закрыть**
- Расширить отчётные сценарии (экспорты, автодоставку) с учётом новых требований клиентского портала.
- Провести финальные smoke-/regression-тесты после обновления отчётности и задокументировать результаты.
