# Targetbot — этап 11: мастер проекта с периодом и расписанием

Проект «CI → GitHub → Cloudflare Workers → Telegram-бот отчётов по рекламе Meta» развивается поэтапно. После каркаса,
регистрации чатов, мастера проектов, расписаний, KPI, автопаузы и учёта оплат добавлен полноценный редактор алертов: из
Telegram можно управлять окнами billing-напоминаний и временем zero-spend контроля, включая кастомные значения и отключение
проверок.
...
- При создании проекта можно сразу выбрать период и расписание; итоговое сообщение показывает, какое расписание установлено.
...
3. Расширить мастер проекта: добавить первичную настройку KPI, статуса биллинга, оплат и алертов прямо при создании.
