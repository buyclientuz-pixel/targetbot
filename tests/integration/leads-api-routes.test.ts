...
test("birllash project sync pulls 4 leads across multiple forms", async () => {
  const env = createEnv();
  const router = createRouter();
  registerLeadWorkerRoutes(router);
  await env.LEADS_KV.put("FORM_IDS:birllash2025", JSON.stringify(["form_primary", "form_secondary"]));

  const originalFetch = globalThis.fetch;
  globalThis.fetch = (async (input: RequestInfo | URL) => {
    const url = typeof input === "string" ? new URL(input) : new URL((input as Request).url);
    if (url.hostname === "graph.facebook.com") {
      const formPath = url.pathname.split("/").at(-2);
      if (formPath === "form_primary") {
        return new Response(
          JSON.stringify({
            data: [
              {
                id: "bir-lead-3",
                created_time: "2025-11-18T15:10:00+0000",
                field_data: [
                  { name: "full_name", values: ["Zamira"] },
                  { name: "phone_number", values: ["+998911111111"] },
                ],
              },
              {
                id: "bir-lead-4",
                created_time: "2025-11-18T16:22:00+0000",
                field_data: [
                  { name: "full_name", values: ["Bekzod"] },
                  { name: "phone_number", values: ["+998922222222"] },
                ],
              },
            ],
          }),
        ...
        return new Response(
          JSON.stringify({
            data: [
              ...
              {
                id: "bir-lead-2",
                created_time: "2025-11-18T14:22:11+0000",
                field_data: [
                  { name: "full_name", values: ["Aziz"] },
                  { name: "phone_number", values: ["+998977777777"] },
                ],
  ...
  try {
    const execution = new TestExecutionContext();
    const syncResponse = await router.dispatch(
      new Request("https://example.com/api/projects/birllash2025/sync-leads"),
      env,
      execution,
    );
    await execution.flush();
    assert.equal(syncResponse.status, 200);
    const syncBody = (await syncResponse.json()) as { success: boolean; imported: number };
    assert.equal(syncBody.imported, 4);

    const leadsResponse = await router.dispatch(
      new Request("https://example.com/api/projects/birllash2025/leads"),
      env,
      new TestExecutionContext(),
    );
    assert.equal(leadsResponse.status, 200);
    ...
    assert.equal(payload.project_id, "birllash2025");
    assert.equal(payload.total, 4);
    assert.deepEqual(
      payload.leads.map((lead) => lead.lead_id),
      ["bir-lead-4", "bir-lead-3", "bir-lead-2", "bir-lead-1"],
    );
    assert.equal(payload.leads[0]?.name, "Bekzod");
  } finally {
    globalThis.fetch = originalFetch;
  }
});
