  ...
  const summary = project.summary
    ? `<div class="mt-4 grid grid-cols-2 gap-3 text-sm md:grid-cols-4">
        <div>
          ...
          <div class="font-semibold">${escapeHtml(formatCurrency(project.summary.spend, project.currency || "USD"))}</div>
        </div>
        <div>
          <div class="text-slate-500">Лиды</div>
          <div class="font-semibold">${escapeHtml(String(project.summary.leads ?? "—"))}</div>
        </div>
        <div>
          <div class="text-slate-500">Клики</div>
          <div class="font-semibold">${escapeHtml(String(project.summary.clicks ?? "—"))}</div>
        </div>
        <div>
          <div class="text-slate-500">CTR</div>
          <div class="font-semibold">${escapeHtml(String(project.summary.ctr ?? "—"))}</div>
        </div>
      </div>`
    : '<p class="mt-4 text-sm text-slate-500">Нет свежей сводки</p>';

  const badgeRow = joinHtml([alerts, silent]);

  const projectIdAttr = escapeHtml(project.id);
  const alertsEnabled =
    project.alerts_enabled === undefined || project.alerts_enabled === null
      ? true
      : Boolean(project.alerts_enabled);
  const silentEnabled = Boolean(project.silent_weekends);
  const toggleAlertsText = alertsEnabled ? "🔕 Выключить алерты" : "🔔 Включить алерты";
    ...
    >
      <div class="grid gap-3 sm:grid-cols-2">
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Название
          <input name="name" type="text" value="${inputValue(project.name)}" class="${formInputClass}" placeholder="Название проекта" />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Статус
          <input name="status" type="text" value="${inputValue(project.status || "")}" class="${formInputClass}" placeholder="active" />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Chat ID
          <input name="chat_id" type="text" value="${inputValue(project.chat_id || "")}" class="${formInputClass}" placeholder="-100123456" />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Telegram
          <input name="chat_username" type="text" value="${inputValue(project.chat_username || "")}" class="${formInputClass}" placeholder="@username" />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Ссылка на чат
          <input name="chat_link" type="text" value="${inputValue(project.chat_link || "")}" class="${formInputClass}" placeholder="https://t.me/..." />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Аккаунт Facebook
          <input name="account_name" type="text" value="${inputValue(project.account_name || "")}" class="${formInputClass}" placeholder="Ad Account" />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          День оплаты
          <input name="billing_day" type="number" value="${inputValue(project.billing_day ?? "")}" class="${formInputClass}" min="1" max="31" />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Менеджер
          <input name="manager" type="text" value="${inputValue(project.manager || "")}" class="${formInputClass}" placeholder="Имя менеджера" />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Портал
          <input name="portal_url" type="text" value="${inputValue(project.portal_url || "")}" class="${formInputClass}" placeholder="https://.../portal" />
        </label>
      </div>
  ...
  const billingForm = `
    ...
    >
      <div class="grid gap-3 sm:grid-cols-2">
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Сумма
          <input name="amount" type="number" step="0.01" value="${inputValue(billing.amount ?? "")}" class="${formInputClass}" placeholder="1200000" />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Валюта
          <input name="currency" type="text" value="${inputValue(billing.currency || project.currency || "")}" class="${formInputClass}" placeholder="USD" />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Последняя оплата
          <input name="last_payment" type="date" value="${inputValue(billing.last_payment || "")}" class="${formInputClass}" />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Следующая оплата
          <input name="next_payment" type="date" value="${inputValue(billing.next_payment || billing.next_payment_date || "")}" class="${formInputClass}" />
        </label>
        <label class="flex flex-col gap-1 text-xs font-semibold uppercase tracking-wide text-slate-400">
          Статус
          <input name="status" type="text" value="${inputValue(billing.status || "")}" class="${formInputClass}" placeholder="paid" />
        </label>
      </div>
      <div class="flex flex-wrap gap-2">
        <button type="submit" class="rounded-lg bg-emerald-500 px-3 py-1.5 text-sm font-semibold text-slate-950 hover:bg-emerald-400">
...
const renderAccountsTab = (projects: ProjectCard[], accounts: MetaAccountInfo[]): string => {
  ...
  const availableChats = projects
    .filter(
      (project) =>
        !project.account_id && (project.chat_link || project.chat_username || project.chat_id)
    )
  ...
  const buildChatButtons = (): string => {
    ...
    return `<div class="flex flex-wrap gap-2" data-account-chat-list>${availableChats
      .map(
        (chat) =>
  ...
  const cards = accounts
    .map((account) => {
      const project =
        projects.find((item) => item.account_id && item.account_id === account.id) || null;
      const hasChat = Boolean(
        project && (project.chat_link || project.chat_username || project.chat_id)
      );
      const spend =
        project && typeof project.summary?.spend === "number"
      ...
      const spendLabel =
        project && project.summary && (project.summary as any).period_label
          ? ` (${escapeHtml(String((project.summary as any).period_label))})`
          : "";
      const spendRow = `<div class="mt-3 text-sm text-slate-300">💰 Потрачено: <span class="font-semibold text-emerald-300">${escapeHtml(spend)}</span>${spendLabel}</div>`;

      const lastUpdate = project?.updated_at || project?.last_sync || account.last_update || null;
      const lastUpdateRow = `<div class="mt-1 text-xs text-slate-500">Обновлено: ${escapeHtml(formatDateTime(lastUpdate))}</div>`;

      const linkedControls =
        hasChat && project
          ? `<div class="mt-4 flex flex-wrap gap-2"><button type="button" data-account-action="open-project" data-project="${escapeHtml(project.id)}" class="rounded-lg bg-emerald-500 px-3 py-2 text-sm font-semibold text-slate-950 shadow-lg shadow-emerald-500/30 hover:bg-emerald-400">Открыть проект</button>${
              project.portal_url
  ...
  const tokenIcon =
    tokenStatus === "ok"
      ...
      : tokenStatus === "expired" || tokenStatus === "missing"
        ...
        : "🟡";
  const tokenIssues =
    token.issues && token.issues.length
  ...
  const rows = tokens
    .map((token) => {
      ...
      const hint = token.hint
        ? `<span class="text-slate-500">(${escapeHtml(token.hint)})</span>`
        : "";
      return `<div class="flex items-center justify-between rounded-xl border border-slate-800 bg-slate-900 px-4 py-3"><div><div class="text-sm font-semibold">${icon} ${escapeHtml(token.name)}</div>${hint}</div><div class="text-sm \${(token.configured ? 'text-emerald-400' : 'text-red-400')}">\${statusText}</div>\${'</div>'}`;
    })
    .join("");
  return `<div class="space-y-3">${rows || '<p class="text-sm text-slate-400">Нет данных</p>'}</div>`;
};
...
const renderStorage = (storage: StorageOverview): string => {
  ...
  const stats = cells
    ...
    .join("");
  const fallback =
    storage.kvFallbacks === null || storage.kvFallbacks === undefined
      ? ""
      : `<p class="mt-3 text-xs text-slate-400">Fallback KV записей: ${escapeHtml(String(storage.kvFallbacks))}</p>`;
  return `<div class="rounded-2xl border border-slate-800 bg-slate-950 p-6">${'<h2 class="text-lg font-semibold">Хранилище</h2>'}<div class="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">${stats}</div>${fallback}</div>`;
};
...
const renderCronStatus = (cron?: CronStatusMap | null): string => {
  ...
  if (entries.length === 0) {
    return `<div class="rounded-2xl border border-slate-800 bg-slate-950 p-6">${'<h2 class="text-lg font-semibold">Крон-задачи</h2>'}<p class="mt-2 text-sm text-slate-500">Отчёты о выполнении крон-задач появятся после первого запуска.</p>${"</div>"}`;
  }
  ...
  const rows = entries
    ...
    .map((entry) => {
      ...
      const lastRunIso =
        entry.last_run && entry.last_run !== "1970-01-01T00:00:00.000Z" ? entry.last_run : null;
      ...
      const lastSuccess =
        entry.last_success && entry.last_success !== "1970-01-01T00:00:00.000Z"
          ...
          : null;
      const failureBadge =
        entry.failure_count && entry.failure_count > 0
...
const renderTabs = (): string => {
  return `<div class="mb-6 flex flex-wrap gap-2">${TAB_CONFIG.map(
    (tab, index) =>
...
export const renderAdminPage = (data: AdminDashboardData): string => {
  const content = joinHtml([renderTabs(), renderTabContent(data)]);
