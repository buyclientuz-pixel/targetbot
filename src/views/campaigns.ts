...
const renderFilters = (projectId: string, period: string, onlyActive: boolean): string => {
  ...
  return (
    '<form method="get" class="flex flex-col gap-3 rounded-xl border border-slate-800 bg-slate-950 p-4 md:flex-row md:items-end">' +
    '<div class="flex flex-col">' +
    '<label class="text-sm text-slate-400">Период</label>' +
    '<select name="period" class="mt-1 rounded-lg border border-slate-700 bg-slate-900 px-3 py-2 text-sm text-slate-100">' +
    optionsHtml +
    '</select>' +
    '</div>' +
    '<label class="flex items-center gap-2 text-sm text-slate-300">' +
    '<input type="checkbox" name="onlyActive" value="1"' + (onlyActive ? ' checked' : '') +
    ' class="h-4 w-4 rounded border-slate-700 bg-slate-900" />' +
    '<span>Только активные</span>' +
    '</label>' +
    '<div class="flex gap-3">' +
    '<button type="submit" class="rounded-lg bg-emerald-500 px-4 py-2 text-sm font-semibold text-slate-950 hover:bg-emerald-400">Фильтр</button>' +
    '<a href="/api/project/' + escapeHtml(projectId) + '/refresh" class="rounded-lg border border-emerald-500 px-4 py-2 text-sm font-semibold text-emerald-400 hover:bg-emerald-500/10">Обновить</a>' +
    '</div>' +
    '</form>'
  );
};
...
const renderTable = (campaigns: CampaignMetric[], currency: string): string => {
  const header =
    '<table class="min-w-full overflow-hidden rounded-xl border border-slate-800 bg-slate-950 text-sm">' +
    '<thead class="bg-slate-900 text-xs uppercase text-slate-400">' +
    '<tr>' +
    '<th class="px-4 py-3 text-left">Статус</th>' +
    '<th class="px-4 py-3 text-left">Название</th>' +
    '<th class="px-4 py-3 text-right">Потрачено</th>' +
    ...
    '<th class="px-4 py-3 text-right">Последняя активность</th>' +
    '</tr>' +
    '</thead>' +
  ...
  const rows = campaigns
    .map((campaign) => {
      ...
      return (
        ...
        '<td class="px-4 py-3 font-medium">' + escapeHtml(campaign.name) + '</td>' +
        '<td class="px-4 py-3 text-right">' + formatCurrency(campaign.spend, currency) + '</td>' +
        '<td class="px-4 py-3 text-right">' + formatNumber(campaign.leads) + '</td>' +
        '<td class="px-4 py-3 text-right">' + formatNumber(campaign.clicks) + '</td>' +
        '<td class="px-4 py-3 text-right">' + formatNumber(campaign.impressions) + '</td>' +
