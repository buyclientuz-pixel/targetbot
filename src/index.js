...
const META_USER_PREFIX = 'tg_user:';
const ADMIN_CALLBACK_PREFIX = 'admin:';
const ADMIN_MENU_ROOT = `${ADMIN_CALLBACK_PREFIX}home`;
const ADMIN_MENU_CHATS = `${ADMIN_CALLBACK_PREFIX}chats`;
const ADMIN_MENU_PROJECTS = `${ADMIN_CALLBACK_PREFIX}projects`;
const ADMIN_MENU_DIAGNOSTICS = `${ADMIN_CALLBACK_PREFIX}diagnostics`;
const ADMIN_MAX_ITEMS = 10;
...
const PROJECT_TEMPLATE = {
  ...
  billing: 'paid',
  campaigns: [],
  kpi: { cpl: null, leads_per_day: null, daily_budget: null },
  weekly: { enabled: true, mode: 'week_today' },
  autopause: { enabled: false, days: 3 },
  alerts: { enabled: true, billing_times: ['10:00', '14:00', '18:00'], no_spend_by: '12:00' },
  anomaly: { cpl_jump: 0.5, ctr_drop: 0.4, impr_drop: 0.5, freq: 3.5 },
  billing_paid_at: null,
  billing_next_at: null,
};
...
class AdminPanel {
  ...
  buildMenu(active = ADMIN_MENU_ROOT) {
    const button = (text, callback) => ({ text, callback_data: callback });
    return {
      inline_keyboard: [
        [button(active === ADMIN_MENU_ROOT ? '📊 Главное ▸' : '📊 Главное', ADMIN_MENU_ROOT)],
        [button(active === ADMIN_MENU_CHATS ? '💬 Чаты ▸' : '💬 Чаты', ADMIN_MENU_CHATS)],
        [button(active === ADMIN_MENU_PROJECTS ? '🗂 Проекты ▸' : '🗂 Проекты', ADMIN_MENU_PROJECTS)],
        [button(active === ADMIN_MENU_DIAGNOSTICS ? '🛠 Диагностика ▸' : '🛠 Диагностика', ADMIN_MENU_DIAGNOSTICS)],
      ],
    };
  }

  async showHome(context) {
    const [chatKeys, projectKeys, metaUsers] = await Promise.all([
      this.storage.listKeys('DB', CHAT_KEY_PREFIX, 200),
      this.storage.listKeys('DB', PROJECT_KEY_PREFIX, 200),
      this.storage.listKeys('DB', META_USER_PREFIX, 50),
    ]);

    const lines = [
      '<b>Админ-панель Targetbot</b>',
      `• Зарегистрированных чатов: <b>${chatKeys.length}</b>`,
      `• Проектов: <b>${projectKeys.length}</b>`,
      `• Подключённых Meta OAuth: <b>${metaUsers.length}</b>`,
    ];

    if (this.config.defaultTimezone) {
      lines.push(`• Таймзона по умолчанию: <code>${escapeHtml(this.config.defaultTimezone)}</code>`);
    }

    if (this.config.workerUrl) {
      lines.push(`• Worker URL: ${escapeHtml(this.config.workerUrl)}`);
    }

    lines.push('', 'Выберите раздел, чтобы посмотреть детали.');

    return this.respond(context, lines.join('\n'), ADMIN_MENU_ROOT);
  }

  async showChats(context) {
    const keys = await this.storage.listKeys('DB', CHAT_KEY_PREFIX, ADMIN_MAX_ITEMS + 1);
    const items = [];

    for (const key of keys.slice(0, ADMIN_MAX_ITEMS)) {
      const record = await this.storage.getJson('DB', key);
      if (!record) continue;
      const title = escapeHtml(record.title || 'Без названия');
      const thread = escapeHtml(record.thread_name || record.thread_id || '—');
      const chatId = escapeHtml(record.chat_id || key.slice(CHAT_KEY_PREFIX.length));
      items.push(`• <code>${chatId}</code> → <code>${thread}</code> — ${title}`);
    }

    const lines = ['<b>Зарегистрированные чаты</b>'];

    if (items.length === 0) {
      lines.push('Пока ни один топик не зарегистрирован. Используйте /register внутри нужного топика.');
    } else {
      lines.push(...items);
      if (keys.length > ADMIN_MAX_ITEMS) {
        lines.push(`… и ещё ${keys.length - ADMIN_MAX_ITEMS}`);
      }
    }
  ...
  async showProjects(context) {
    const keys = await this.storage.listKeys('DB', PROJECT_KEY_PREFIX, ADMIN_MAX_ITEMS + 1);
    const items = [];

    for (const key of keys.slice(0, ADMIN_MAX_ITEMS)) {
      const record = await this.storage.getJson('DB', key);
      if (!record) continue;
      const code = escapeHtml(record.code || key.slice(PROJECT_KEY_PREFIX.length));
      const act = escapeHtml(record.act || 'act_...');
      const chatId = record.chat_id ? `<code>${escapeHtml(record.chat_id)}</code>` : 'чат?';
      const period = escapeHtml(record.period || PROJECT_TEMPLATE.period);
      items.push(`• <b>${code}</b> → ${act} • ${chatId} • период: <code>${period}</code>`);
    }

    const lines = ['<b>Проекты</b>'];

    if (items.length === 0) {
      lines.push('Проекты ещё не созданы. Следующий этап добавит мастер создания и редактирования.');
    } else {
      lines.push(...items);
      if (keys.length > ADMIN_MAX_ITEMS) {
        lines.push(`… и ещё ${keys.length - ADMIN_MAX_ITEMS}`);
    ...
    lines.push('', 'Структура проекта по умолчанию:');
    const templatePreview = escapeHtml(JSON.stringify(PROJECT_TEMPLATE, null, 2));
    lines.push(`<pre><code>${templatePreview}</code></pre>`);

    return this.respond(context, lines.join('\n'), ADMIN_MENU_PROJECTS);
  }

  async showDiagnostics(context) {
    const lines = ['<b>Диагностика</b>'];

    const tokenStatus = this.config.botToken ? 'найден' : 'отсутствует';
    lines.push(`• Токен Telegram: ${tokenStatus}`);
    lines.push(`• Администраторов: ${this.config.adminIds.size}`);

    try {
      const info = this.bot.telegram.isUsable ? await this.bot.telegram.getWebhookInfo() : null;
      if (info) {
        const url = escapeHtml(info.url || '—');
        const lastError = info.last_error_message ? escapeHtml(info.last_error_message) : null;
        lines.push(
          [
            '• Webhook:',
            `  URL: ${url}`,
        ...
        lines.push('• Webhook: нет данных');
      }
    } catch (error) {
      lines.push(`• Webhook: ошибка — ${error.message}`);
    }

    lines.push('', 'Логи вебхука доступны через /health?telegram_logs=1.');

    return this.respond(context, lines.join('\n'), ADMIN_MENU_DIAGNOSTICS);
  }

  async handleCallback(context, data) {
    switch (data) {
      case ADMIN_MENU_ROOT:
        await this.showHome(context);
        return true;
      case ADMIN_MENU_CHATS:
        await this.showChats(context);
        return true;
      case ADMIN_MENU_PROJECTS:
        await this.showProjects(context);
        return true;
      case ADMIN_MENU_DIAGNOSTICS:
        await this.showDiagnostics(context);
        return true;
      default:
        return false;
    }
  ...
  async respond(context, text, active) {
    const markup = this.buildMenu(active);
    if (context instanceof BotCallbackContext) {
      return context.updateMessage(text, { reply_markup: markup });
    }
    return context.reply(text, { reply_markup: markup });
  }
}
...
class BotCallbackContext {
  constructor(bot, update, callback) {
    this.bot = bot;
    this.update = update;
  ...
  get callbackId() {
    return this.callback?.id ?? null;
  }

  get message() {
    return this.callback?.message ?? null;
  }
  ...
  get user() {
    return this.callback?.from ?? null;
  }
  ...
  async updateMessage(text, options = {}) {
    return this.bot.safeEditCallback(this.callback, text, options);
  }

  async sendMessage(text, options = {}) {
    const targetChatId = this.chatId ?? this.userId;
    if (!targetChatId) {
      throw new Error('Cannot determine chat for callback message');
    }
...
class TelegramBot {
  ...
  registerDefaultCommands() {
    ...
    this.registerCommand('admin', async (context) => {
      ...
      await this.adminPanel.showHome(context);
    });
  ...
  async handleCallback(update, callback) {
    ...
    let ackError = null;
    try {
      await this.telegram.answerCallbackQuery({ callback_query_id: id });
    } catch (error) {
      ackError = error;
      console.error('Callback ack failed', error);
    }

    const context = new BotCallbackContext(this, update, callback);

    try {
      if (data.startsWith(ADMIN_CALLBACK_PREFIX)) {
        const handled = await this.adminPanel.handleCallback(context, data);
        this.queueLog({ kind: 'callback', status: handled ? 'ok' : 'ignored', data });
        if (!handled) {
          await context.sendMessage('⚠️ Раздел пока не реализован.');
        }
        return { handled };
      }

      if (data) {
        await context.sendMessage('⚠️ Этот callback ещё не поддерживается.');
      }
      const logEntry = { kind: 'callback', status: 'ignored', data };
      if (ackError) {
        logEntry.ack_error = ackError.message || String(ackError);
      }
  ...
  async safeEditCallback(callback, text, options = {}) {
    if (!callback) {
    ...
    if (payload.chat_id && payload.message_id) {
      try {
        return await this.telegram.editMessageText(payload);
      } catch (error) {
        console.error('editMessageText failed', error);
      }
    }

    const fallbackChatId = message?.chat?.id ?? callback?.from?.id;
    if (!fallbackChatId) {
      throw new Error('Cannot resolve chat for callback response');
    }
    ...
    return this.telegram.sendMessage(fallbackPayload);
  }
