...
> ⚙️ И по расписанию, и при ручном запуске отчёты собираются только за полностью завершённый предыдущий день. Сообщение, отправленное 19-го числа, показывает блоки «Вчера/Позавчера» с цифрами за 18/17 числа, чтобы исключить неполные сутки.
